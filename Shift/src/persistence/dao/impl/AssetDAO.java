...
public class AssetDAO extends AbstractDAO<Asset> implements DAO<Asset> {
	...
	public Asset create() {
		...
		try {
			...
			pre = con.prepareStatement("insert into asset (einzelpreis, bezeichnung) values (?, ?);");
	...
	public void persist(Asset entity) {
		...
		try {
			PreparedStatement pre;
			pre = con.prepareStatement("update asset SET assetID = ?, billID = ?, einzelpreis = ?, bezeichnung = ? WHERE assetID = " + entity.getId());
